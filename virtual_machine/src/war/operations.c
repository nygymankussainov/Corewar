...
void				add_to_arena(uint8_t *arena, uint16_t position, uint32_t code)
{
	arena[position] = code << 24;
	arena[position + 1] = code << 16;
	arena[position + 2] = code << 8;
	arena[position + 3] = code;
}
...
void				op_live(t_corewar *vm, t_carriage *carriage, int8_t *arg_code)
{
	...
	//изменила тип player_code с uint8_t на int32_t	т.к считываем 4 байта и число возможно будет отрицательное
	vm->live_count++;
    carriage->cycle_was_live = vm->total_cycles + vm->current_cycles;
	player_code = 256 - return_bytes(vm->arena, carriage->position + 1, 4);
	// вычитаю из 256
	if (player_code > 0 && player_code <= vm->number_of_players) //заменила MAX_PLAYERS на vm->number_of_players
    {
...
void				op_st(t_corewar *vm, t_carriage *carriage, int8_t *arg_code)
{
	...
	else
	{
		...
		add_to_arena(vm->arena, carriage->position + arg1 % IDX_MOD, carriage->registers[reg_num - 1]);
	}
}
