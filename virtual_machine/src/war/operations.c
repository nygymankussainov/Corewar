...
void				op_st(t_corewar *vm, t_carriage *carriage, int8_t *arg_code)
{
	...
	{
		arg1 = return_bytes(vm->arena, pos, IND_SIZE);
		add_to_arena(vm->arena, (carriage->position + arg1 % IDX_MOD) % MEM_SIZE,
			carriage->registers[reg_num], carriage);
	}
}
...
void				op_sti(t_corewar *vm, t_carriage *carriage, int8_t *arg_code)
{
	...
	if (arg_code[1] == 1)
	{	
		arg1 = return_bytes(vm->arena, pos, T_REG);
		++pos;
	}
	...
	else
	{
		arg1 = return_bytes(vm->arena, pos, IND_SIZE);
		pos += 2;
		arg1 = return_bytes(vm->arena, carriage->position + arg1 % IDX_MOD, DIR_SIZE);
	}
	arg2 = (arg_code[2] == 1) ? return_bytes(vm->arena, pos, T_REG) : return_bytes(vm->arena, pos, carriage->operation->t_dir_size);
	add_to_arena(vm->arena, carriage->position + (arg1 + arg2) % IDX_MOD, carriage->registers[reg_num], carriage);
}
...
void				op_lld(t_corewar *vm, t_carriage *carriage, int8_t *arg_code)
{
	...
	else
	{
		arg1 = return_bytes(vm->arena, pos, IND_SIZE);
		pos += 2;
		arg1 = return_bytes(vm->arena, carriage->position + arg1, DIR_SIZE);
	}
	reg_num = vm->arena[pos % MEM_SIZE].value;
	carriage->registers[reg_num] = arg1;
}
