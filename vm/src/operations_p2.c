...
/*   Updated: 2019/12/02 09:40:25 by vhazelnu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		xor(t_vm *vm, t_carr *carr, t_player *player)
{
	...
	while (i < carr->op->args_number - 1)
	{
		if (vm->args_type[i] == REG_CODE)
			value ^= carr->reg[vm->args[i] - 1];
		else if (vm->args_type[i] == DIR_CODE)
			value ^= (int)vm->arena[(carr->pos + i) % MEM_SIZE];
		else
			value ^= get_ind_value(vm, carr, vm->args[i] % IDX_MOD, REG_SIZE);
		++i;
	}
