...
/*   Updated: 2019/11/11 18:29:26 by vhazelnu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	validate_register(char **line, t_major *major, t_token **token)
{
	...
	if ((*line)[COL] == '-')
		print_error(line, Syntax, "T_REG must be grater than 0 ", major);
	...
	if ((*line)[COL] && (*line)[COL] != SEPARATOR_CHAR &&
		...
		print_error(line, Syntax,
			"Argument T_REG is not well formated at ", major);
	i = skip_zeros(*line, i);
